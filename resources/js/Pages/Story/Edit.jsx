...
export default function Edit({ story, auth }) {
    ...
    const [preview, setPreview] = useState(story.image_path ? story.image_path : null);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route("story.update", story.id), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <AuthenticatedLayout
            ...
            header={<h2 className="text-2xl font-bold text-gray-800">Edit Story</h2>}
        >
            <div className="max-w-3xl mx-auto mt-10 bg-white p-8 rounded-xl shadow-lg">
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    <button
                        type="submit"
                        disabled={processing}
                        className={`bg-blue-600 text-white font-semibold px-6 py-2 rounded-md hover:bg-blue-700 transition ${processing ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
